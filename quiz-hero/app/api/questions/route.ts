...
async function askWithRetry(systemPrompt: string, userPrompt: string, retries = 3): Promise<object> {
    ...
    try {
     ...
     console.log(result)
      ...
      if (isValidJSONResponse(json)) {
        ...
        throw new Error("Received invalid JSON structure from Gemini.");
      }
    } catch (e: any) {
      if (i === retries - 1) {
  ...
  throw new Error("Unexpected error: askWithRetry loop completed without success or final throw.");
}
...
export const POST = async (req: NextRequest): Promise<NextResponse> => {
  try {
    ...
    return NextResponse.json(
      { error: err.message || "Failed to generate quiz due to an internal server error." },
      { status: 500 }
    );
  }
};
