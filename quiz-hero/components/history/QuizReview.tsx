...
const QuestionCard = React.memo(({ q, index, total }: { q: Question; index: number; total: number }) => {
    ...
    return (
        <Card className="w-full bg-white border-gray-200 shadow-xl transition-all">
            ...
            <CardContent>
                <div className="grid gap-4">
                    {Array.isArray(options) && options.map((option: string, i: number) => {
    ...
    return (
        <div className="space-y-8">
            {game.questions.map((q, index) => (
                <QuestionCard key={q.id} q={q} index={index} total={game.questions.length} />
            ))}
        </div>
    );
}
